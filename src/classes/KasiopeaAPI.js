    ...
    login(email, password) {
        return new Promise((resolve, reject) => {
            ...
            bodyFormData.submit("https://kasiopea.matfyz.cz/auth/login.cgi?done=1", (err, res) => {
                if (err) reject(err)
                if(res.headers["set-cookie"].length<1) reject("Wrong e-mail or password, if you are sure, that your e-mail/password combination is correct, please open an issue.")
