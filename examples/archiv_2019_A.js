...
Agent.login("email", "heslo").then(async (l) => {
    l.eoh = 2; // funguje pro obě obtížnosti
    ...
    let j = 1
    let reseni = ""
    while (i < pocetProblemu) { // dokud je i menší než počet příkladů
        ...
        nulaIndex.forEach(el => {
            if (el == 0) { // pokud je nula na začátku, nastaví na 1
                problem[el] = (1).toString()
            } else { // jinak nastaví na předchozí hodnotu + 1
        ...
        while (finale < problem.length) { // zkontroluje, jestli jdou čísla po sobě, pokud ne, nastaví 'jeTamChyba' na true
            if (finale == problem.length - 1) break;
            if ((parseInt(problem[finale]) + 1 !== parseInt(problem[finale + 1]))) jeTamChyba = true
            finale++
        }

        if (jeTamChyba) { // pokud je chyba, vloží do proměnné výsledku 'NE'
